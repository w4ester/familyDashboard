/**
 * Family Dashboard MCP Filesystem Server - Secure Version
 * 
 * This server provides LLMs (like Claude) with controlled access to the filesystem
 * It runs with HTTPS using self-signed certificates for secure local operation
 */
...
httpServer.listen(HTTP_PORT, () => {
  ...
  logger.info(`Access at: http://localhost:${HTTP_PORT}`);
});
...
if (USE_HTTPS) {
  ...
  if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
    ...
    httpsServer.listen(HTTPS_PORT, () => {
      ...
      logger.info(`Access at: https://localhost:${HTTPS_PORT}`);
      logger.info('Using self-signed certificate - you may need to accept security warnings in your browser');
    });
    servers.push(httpsServer);
  } else {
    logger.warn('SSL certificates not found. Run generate-ssl.sh to create them.');
